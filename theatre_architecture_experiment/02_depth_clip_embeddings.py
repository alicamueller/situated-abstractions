...
np.save(OUT_DIR / "depth_embeddings.npy", embeddings)

with open(OUT_DIR / "depth_filenames.json", "w", encoding="utf-8") as f:
    json.dump(filenames, f, ensure_ascii=False, indent=2)

print("\nDone.")
print(f"Saved: {OUT_DIR / 'depth_embeddings.npy'}")
print(f"Saved: {OUT_DIR / 'depth_filenames.json'}")
