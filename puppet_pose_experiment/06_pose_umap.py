...
EMB_PATH = Path("embeddings/pose_embeddings.npy")
FILE_PATH = Path("embeddings/pose_filenames.json")
IMAGE_DIR = Path("pose_renders")
OUT_PATH = Path("outputs/pose_umap_nooverlap.png")
...
# Re-normalize so everything stays visible
pts[:, 0] = (pts[:, 0] - pts[:, 0].min()) / (pts[:, 0].max() - pts[:, 0].min() + 1e-9)
pts[:, 1] = (pts[:, 1] - pts[:, 1].min()) / (pts[:, 1].max() - pts[:, 1].min() + 1e-9)
